...
class Eventos {
	...
	static String enviarEvento(String xml, String tipoEvento, boolean valida, Tipo tipo) throws NfeException {
		...
		try {
			...
			if (tipoEvento.equals(ConstantesUtil.EVENTO.MANIFESTACAO)) {
				url = WebServiceUtil.getUrl(Tipo.NFE, ConstantesUtil.SERVICOS.MANIFESTACAO);
			} else {
