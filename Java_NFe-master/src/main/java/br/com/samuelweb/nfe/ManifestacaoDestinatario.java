...
class ManifestacaoDestinatario {
	...
	static TRetEnvEvento eventoManifestacao(String chave, TipoManifestacao manifestacao, String cnpj, String data,
			String motivo) throws NfeException {
		...
		try {
			...
			infEvento.setTpAmb(configuracoesNfe.getAmbiente().toString());
			...
			xml = Eventos.enviarEvento(xml, ConstantesUtil.EVENTO.MANIFESTACAO, false, Tipo.NFE);
