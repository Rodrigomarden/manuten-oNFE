...
public class WebServiceUtil {
	...
	public static String getUrl(Tipo tipo, String servico) throws NfeException {

		try {

			ConfiguracoesIniciaisNfe config = ConfiguracoesIniciaisNfe.getInstance();
			String secao = tipo + "_" + config.getEstado() + "_"
					+ (config.getAmbiente().equals(Ambiente.HOMOLOGACAO) ? "H" : "P");
			...
			if (servico.equals(ConstantesUtil.SERVICOS.DISTRIBUICAO_DFE)
				...
				secao = config.getAmbiente().equals(Ambiente.HOMOLOGACAO) ? "NFe_AN_H" : "NFe_AN_P";
				...
				if (config.getEstado().equals(Estados.GO) || config.getEstado().equals(Estados.AM)
					...
					secao = tipo + "_SVRS_" + (config.getAmbiente().equals(Ambiente.HOMOLOGACAO) ? "H" : "P");
					// SVC-AN
				} else {
					secao = tipo + "_SVAN_" + (config.getAmbiente().equals(Ambiente.HOMOLOGACAO) ? "H" : "P");
				}
			}
