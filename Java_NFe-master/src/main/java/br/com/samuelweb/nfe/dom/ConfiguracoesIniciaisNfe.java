...
public final class ConfiguracoesIniciaisNfe {
	...
	// Construtor Privado
	private ConfiguracoesIniciaisNfe(Estados estado, Ambiente ambiente, Certificado certificado, String pastaSchemas) {
	...
	public static ConfiguracoesIniciaisNfe iniciaConfiguracoes(Estados estado, Ambiente ambiente,
			Certificado certificado, String pastaSchemas) {
		...
		System.out.println(
				"Ambiente: " + (ambiente.equals(Ambiente.PRODUCAO) ? "Produção" : "Homologação") + " - Estado: " + estado.getNome());
		return instance;
	}

	public static ConfiguracoesIniciaisNfe iniciaConfiguracoes(Estados estado, Ambiente ambiente,
			Certificado certificado, String pastaSchemas, Boolean log) {
		new ConfiguracoesIniciaisNfe(estado, ambiente, certificado, pastaSchemas);
		if (log) {
			System.out.println("Api Java Nfe Versão 4.00.4 - Samuel Olivera - samuk.exe@hotmail.com");
			System.out.println("Certificado: " + certificado.getTipo().toUpperCase() + " - "
					+ certificado.getNome().toUpperCase() + " - Vencimento: " + certificado.getVencimento());
			System.out.println("Ambiente: " + (ambiente.equals(Ambiente.PRODUCAO) ? "Produção" : "Homologação") + " - Estado: "
					+ estado.getNome());
		}
		return instance;
	}
